...
async fn add_and_remove_certificate_authority() {
    ...
    let cleanup =
        start_krill_with_default_test_config(true, true, false, false).await;
    ...
    assert!(
        ca_contains_resources(&testbed_ca_handle, &expected_resources).await
    );

    // verify that the testbed publisher has been created
    assert_eq!(
        testbed_ca_handle,
        publisher_details(testbed_ca_handle.convert())
            .await
            .handle()
            .convert()
    );
    ...
    assert!(
        get_ok(&format!("{}testbed/enabled", KRILL_SERVER_URI), None)
            .await
            .is_ok()
    );
    ...
    let parent_response: idexchange::ParentResponse =
    ...
    let parent_ca_req = ParentCaReq::new(
    ...
    let publisher_found =
        publishers.publishers().iter().any(|ps: &PublisherSummary| {
            ps.handle().as_str() == dummy_ca_handle.as_str()
        });
    ...
    // -------------------------------------------------------------------------
    let org_tal = get_text(&format!("{}ta/ta.tal", KRILL_SERVER_URI), None)
        .await
        .unwrap();
    let renamed_tal =
        get_text(&format!("{}testbed.tal", KRILL_SERVER_URI), None)
            .await
            .unwrap();
