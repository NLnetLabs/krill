...
fn benchmark() {
    ...
    wait_for_nr_cas_under_testbed(server.client(), cas);
    // We expect all CAs, plus the testbed and the ta as publishers
    wait_for_nr_cas_under_publication_server(server.client(), cas + 2);

    server.abort();
}

fn wait_for_nr_cas_under_testbed(
    client: &KrillClient,
    nr: usize
) {
    let handle = common::ca_handle("testbed");
    for _ in 0..300 {
        if client.ca_details(&handle).unwrap().children().len() == nr {
            return;
        }
...
fn wait_for_nr_cas_under_publication_server(
    client: &KrillClient,
    publishers_expected: usize,
) {
    let mut publishers_found = client.publishers_list().unwrap()
        .publishers().len();
    for _ in 0..300 {
        ...
        common::sleep_seconds(1);
        publishers_found = client.publishers_list().unwrap()
            .publishers().len();
    }
