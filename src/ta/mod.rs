...
mod tests {
    ...
    async fn init_ta() {
        ...
        ta_proxy_store.add(proxy_init).await.unwrap();
        ...
        let repository = {
            ...
            let repo_key_id = signer.create_key().await.unwrap();
            let repo_key = signer.get_key_info(&repo_key_id).await.unwrap();
        ...
        let mut proxy = ta_proxy_store.command(add_repo_cmd).await.unwrap();
        ...
        let mut ta_signer = ta_signer_store.add(signer_init_cmd).await.unwrap();
        ...
        proxy = ta_proxy_store.command(add_signer_cmd).await.unwrap();
        ...
        proxy = ta_proxy_store.command(make_publish_request_cmd).await.unwrap();

        let signed_request = proxy.get_signer_request(timing, &signer).await.unwrap();
