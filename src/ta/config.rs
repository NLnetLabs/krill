...
impl Config {
    ...
    fn parse_str(s: &str) -> Result<Self, ConfigError> {
        let mut config: Config =
            toml::from_str(s).map_err(|err| {
                ConfigError::Other(
                    format!("Error parsing config file: {err}")
                )
            })?;
    ...
    pub fn signer(&self) -> Result<Arc<KrillSigner>, ConfigError> {
        ...
        let signer = KrillSignerBuilder::new(&self.storage_uri, probe_interval, &self.signers)
            ...
            .build()
            .map_err(|e| ConfigError::Other(format!("Could not create KrillSigner: {}", e)))?;
    ...
    fn init_logging(&self) -> Result<(), ConfigError> {
        match self.log_type {
            ...
            LogType::Syslog => Err(ConfigError::other("syslog is not supported for the TA client")),
        }
    }
    ...
    fn file_logger(&self) -> Result<(), ConfigError> {
        ...
        let log_file = fern::log_file(path)
            .map_err(|e| ConfigError::Other(format!("Failed to open log file '{}': {}", path.display(), e)))?;
