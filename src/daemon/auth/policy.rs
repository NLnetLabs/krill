...
use crate::{
    ...
    constants::{
        ACTOR_DEF_ADMIN_TOKEN, ACTOR_DEF_ANON, ACTOR_DEF_KRILL,
        ACTOR_DEF_TESTBED,
    },
    daemon::{
        auth::{
...
impl AuthPolicy {
    pub fn new(config: Arc<Config>) -> KrillResult<Self> {
        ...
        // Register both the Permission enum as a Polar class and its variants
        // as Polar constants. The former is useful for writing Polar
        // rules that only match on actual Krill Permissions, not on arbitrary
        // strings, e.g. `allow(actor, action: Permission, resource)`.
        // The latter is useful when writing rules that depend on a
        // specific permission, e.g. `if action = CA_READ`. Without the
        // variants as constants we would have to create a
        // new Permission each time, converting from a string to the
        // Permission type, e.g. `action = new Permission("CA_READ")`.
        ...
        // Load built-in Polar authorization policy rules from embedded
        // strings
        Self::load_internal_policy(
            &mut oso,
            include_bytes!("../../../defaults/roles.polar"),
            "roles",
        )?;
        Self::load_internal_policy(
            &mut oso,
            include_bytes!("../../../defaults/rules.polar"),
            "rules",
        )?;
        Self::load_internal_policy(
            &mut oso,
            include_bytes!("../../../defaults/aliases.polar"),
            "aliases",
        )?;
        Self::load_internal_policy(
            &mut oso,
            include_bytes!("../../../defaults/rbac.polar"),
            "rbac",
        )?;
        Self::load_internal_policy(
            &mut oso,
            include_bytes!("../../../defaults/abac.polar"),
            "abac",
        )?;

        // Load additional policy rules from files optionally provided by the
        // customer
        Self::load_user_policy(config, &mut oso)?;

        // Sanity check: Verify the roles assigned to the built-in actors are
        // as expected.
        ...
        Self::exec_query(
            &mut oso,
            r#"actor_has_role(Actor.builtin("krill"), "admin")"#,
        )?;

        // The "admin-token" built-in actor is used for logins using the admin
        // token (aka the "admin_token" set in the config file or via env
        // var). This actor should have the "admin" role.
        Self::exec_query(
            &mut oso,
            r#"actor_has_role(Actor.builtin("admin-token"), "admin")"#,
        )?;
        ...
        // does not have any role (represented by the _ placeholder in Oso
        // Polar syntax).
        Self::exec_query(
            &mut oso,
            r#"not actor_has_role(Actor.builtin("anon"), _)"#,
        )?;
        ...
        // actions are grouped into a "testbed" role. The "testbed" actor
        // should have the "testbed" role.
        Self::exec_query(
            &mut oso,
            r#"actor_has_role(Actor.builtin("testbed"), "testbed")"#,
        )?;

        Ok(AuthPolicy { oso: Arc::new(oso) })
    }

    pub fn is_allowed<U, A, R>(
        &self,
        actor: U,
        action: A,
        resource: R,
    ...
    where
        ...
        R: ToPolar,
    {
        self.oso.is_allowed(actor, action, resource).map_err(|err| {
            Error::custom(format!(
                "Internal error while checking access against policy: {}",
                err
            ))
        })
    }

    fn load_internal_policy(
        oso: &mut Oso,
        bytes: &[u8],
        fname: &str,
    ) -> KrillResult<()> {
        trace!("Loading Polar policy '{}'", fname);
        oso.load_str(std::str::from_utf8(bytes).map_err(|err| {
            Error::custom(format!(
                "Internal Polar policy '{}' is not valid UTF-8: {}",
                fname, err
            ))
        })?)
        .map_err(|err| {
    ...
    fn exec_query(oso: &mut Oso, query: &str) -> KrillResult<()> {
        oso.query(query).map_err(|err| {
            Error::custom(format!(
                "The Polar self check query '{}' failed: {}",
                query, err
            ))
        })?;
        Ok(())
...
// Allow our "no resource" type to match the "nil" in Oso policy rules by
// making it convertible to the Rust type Oso uses when registering the nil
// constant. We can't use Option::<PolarValue>::None directly as it doesn't
// implement the Display trait which we depend on in non-trace level logging
// in `fn Actor::is_allowed()`.
//
...
impl PolarClass for Actor {
    fn get_polar_class() -> oso::Class {
        Self::get_polar_class_builder()
            ...
            .set_equality_check(|left: &Actor, right: &Actor| {
                left.name() == right.name()
            })
            .add_attribute_getter("name", |instance| {
            ...
            .add_class_method("builtin", |name: String| -> Actor {
                match name.as_str() {
                    ...
                    "admin-token" => {
                        Actor::actor_from_def(ACTOR_DEF_ADMIN_TOKEN)
                    }
                    "testbed" => Actor::actor_from_def(ACTOR_DEF_TESTBED),
                    _ => panic!("Unknown built-in actor name '{}'", name),
                }
            })
            // method to do a "contains" test, either get rid of this if the
            // Oso Polar "in" operator will suffice or move this
            // to a separate Polar Class called Util and name the
            // method "contains".
            .add_class_method(
                "is_in",
                |name: String, names: Vec<String>| -> bool {
                    names.contains(&name)
                },
            )
...
impl PolarClass for Permission {
    fn get_polar_class() -> oso::Class {
        Self::get_polar_class_builder()
            ...
            .set_equality_check(|left: &Permission, right: &Permission| {
                *left == *right
            })
            .build()
    }
