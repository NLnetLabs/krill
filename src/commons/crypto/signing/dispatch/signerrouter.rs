...
pub struct SignerRouter {
    ...
    ///   - Random numbers can only be generated by the default signer if it supports it. See `random_signer`
    ///     below.
...
impl SignerRouter {
    ...
    pub(crate) fn get_mapper(&self) -> Arc<SignerMapper> {
...
impl Signer for SignerRouter {
    ...
    fn rand(&self, target: &mut [u8]) -> Result<(), Self::Error> {
        ...
        if self.default_signer.supports_random() {
            ...
            self.random_signer.rand(target)
        }
    }
}
...
pub mod tests {
    ...
    fn create_signer_router(all_signers: &[Arc<SignerProvider>], signer_mapper: Arc<SignerMapper>) -> SignerRouter {
        SignerRouter {
            ...
            random_signer: all_signers[0].clone(),
