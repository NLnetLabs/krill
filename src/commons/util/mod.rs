...
pub trait AllowedUri {
    ...
    fn allowed_uri(&self, testmode: bool) -> bool {
        if testmode {
            true
        } else {
            seems_global_auth(self.authority())
        }
    }
}
...
mod tests {
    ...
    use crate::commons::util::seems_global_auth;

    #[test]
    fn check_uri_seems_global() {
        // Does not seem global
        assert!(!seems_global_auth("localhost"));
        assert!(!seems_global_auth("0.0.0.0"));
        assert!(!seems_global_auth("127.0.0.1"));
        assert!(!seems_global_auth("127.0.0.1:873"));
        assert!(!seems_global_auth("1.2.3.4"));
        assert!(!seems_global_auth("::"));
        assert!(!seems_global_auth("::1"));
        assert!(!seems_global_auth("[::1]:873"));
        assert!(!seems_global_auth("2001:0db8:85a3:0000:0000:8a2e:0370:7334"));

        // Looks ok
        assert!(seems_global_auth("localghost"));
        assert!(seems_global_auth("rpki.bla"));
    }
}
