...
pub trait AuthProvider: Send + Sync {
    ...
    /// by the same AuthProvider that issued it.
    fn login(&self, request: &hyper::Request<hyper::Body>) -> KrillResult<ActorDef>;
...
`krillc` and direct API clients only use `authenticate()`. They already possess the correct bearer token and only need
to present it to Krill along with each request to authenticate themselves and their identity is implicitly tied to their
authentication details; that is, we cannot know who they actually are, we only know they are a client in possession of
the master API token and for backward compatibility we grant them the `admin` role. However, even the
`MasterTokenAuthProvider` implements all four functions because Lagosta has no explicit knowledge of which
`AuthProvider` is in use and no explicit logic for the master API token authentication case.
